...
void Camera::RotateYForBox(float angle)
{
	XMMATRIX rotation = XMMatrixRotationY(angle);

	XMStoreFloat3(&m_right, XMVector3TransformNormal(XMLoadFloat3(&m_right1), rotation));
	XMStoreFloat3(&m_up, XMVector3TransformNormal(XMLoadFloat3(&m_up1), rotation));
	XMStoreFloat3(&m_look, XMVector3TransformNormal(XMLoadFloat3(&m_look1), rotation));
}
