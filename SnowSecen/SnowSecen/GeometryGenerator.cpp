...
{
	UINT start = static_cast<UINT>(mesh.vertices.size());
	...
	UINT center = static_cast<UINT>(mesh.vertices.size()) - 1;
...
void GeometryGenerator::CreateSphere(float radius, int slice, int stack, MeshData &mesh)
{
	...
	UINT tmp(0);
	int start1 = 0;
	int start2 = static_cast<int>(mesh.vertices.size()) - vertsPerRow - 2;
	int top = size;
	int bottom = size + 1;
	for (int i = 0; i < slice; ++i)
	{
...
void GeometryGenerator::CreateTorus(float diameter, float thickness, int tessellation, MeshData &mesh)
{
	mesh.vertices.clear();
	mesh.indices.clear();
	...
	int nVerts = stride * stride;
	int nIndices = nVerts * 6;
	...
	for (int i = 0; i <= tessellation; i++)
	{
		...
		for (int j = 0; j <= tessellation; j++)
		{
			...
			// Create a vertex.
			XMVECTOR normal = DirectX::XMVectorSet(dx, dy, 0, 0);
			XMVECTOR position = normal * thickness / 2;
			XMVECTOR textureCoordinate = DirectX::XMVectorSet(u, v, 0, 0);

			position = XMVector3Transform(position, transform);
			normal = XMVector3TransformNormal(normal, transform);

			XMStoreFloat3(&mesh.vertices[vIndex].pos, position);
			XMStoreFloat3(&mesh.vertices[vIndex].normal, DirectX::XMVector3Normalize(normal));
			XMStoreFloat3(&mesh.vertices[vIndex].tangent, DirectX::XMVector3Normalize(XMVector3Orthogonal(normal)));
			mesh.vertices[vIndex].tex = XMFLOAT2(u, v);
			vIndex++;
			...
			mesh.indices[iIndex++] = nextI * stride + j;
			mesh.indices[iIndex++] = i * stride + nextJ;
			mesh.indices[iIndex++] = i * stride + j;

			mesh.indices[iIndex++] = nextI * stride + j;
			mesh.indices[iIndex++] = nextI * stride + nextJ;
			mesh.indices[iIndex++] = i * stride + nextJ;
		}
	}
}